      ...
      <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded bg-white">
        <div class="rounded-t mb-0 px-4 py-3 border-0">
          <div class="text-center flex justify-between">
            <h6 class="text-gray-800 text-xl font-bold">Budget List</h6>
        ...
        <div class="block w-full overflow-x-scroll">
          ...
          <table class="items-center w-full bg-transparent border-collapse datatables">
            ...
            <tbody>
              <?php foreach ($budget_list as $key => $value): ?>
                <tr>
                  <td class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200"><?php echo $value['year_budget'] ?></td>
                  <td class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200"><?php echo $value['month_budget'] ?></td>
                  <td class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200"><?php echo $value['cost_budget'] ?></td>
                  <td class="px-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 whitespace-no-wrap font-semibold text-left bg-gray-100 text-gray-600 border-gray-200">
                    ...
                    <div class="hidden bg-white text-base z-50 float-left py-2 list-none text-left rounded shadow-lg min-w-48" id="table-dark-<?php echo $key ?>-dropdown">
                      <a href="<?php echo base_url() ?>budget/budget_edit/<?php echo $value['id'] ?>" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">Edit</abs
                      ><a href="<?php echo base_url() ?>budget/budget_delete_process/<?php echo $value['id'] ?>" onclick="return confirm('Are You Sure?')" class="text-sm py-2 px-4 font-normal block w-full whitespace-no-wrap bg-transparent text-gray-800">Delete</a>
                    </div>
                  </td>
                </tr>
              <?php endforeach; ?>
